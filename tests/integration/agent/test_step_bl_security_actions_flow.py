...
class TestSessionRevokeFlow:
    def test_revoke_active_session(self):
        ...
        v = AdminSecurityActionService.validate_revoke_session(
            d, confirm=True, actions_enabled=True
        )
        assert v.ok
...
class TestIPRuleFlow:
    def test_create_and_evaluate_block(self):
        ...
        rule = AdminSecurityActionService.build_ip_rule_dict(
            "5.5.5.5", "block", "suspicious", "admin1"
        )
        r = AdminSecurityActionService.evaluate_ip_access(
            "5.5.5.5", [rule], enforcement_enabled=True
        )
    ...
    def test_enforcement_off_advisory(self):
        ...
        r = AdminSecurityActionService.evaluate_ip_access(
            "5.5.5.5", [rule], enforcement_enabled=False
        )
    ...
    def test_watch_rule(self):
        ...
        r = AdminSecurityActionService.evaluate_ip_access(
            "3.3.3.3", [rule], enforcement_enabled=True
        )
