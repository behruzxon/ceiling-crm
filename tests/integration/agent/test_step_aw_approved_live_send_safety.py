...
def _rec(*, status="approved", msg="Salom!", user_id=123, risk="low",
         channel="user_dm", action="send_user_reply", expires_minutes=30,
         executed_at=None, failed_at=None, execution_id="test-001"):
    now = datetime.now(UTC)
    r = MagicMock()
    r.execution_id = execution_id
    r.telegram_user_id = user_id
    r.action = action; r.mode = "live"; r.status = status
    r.risk_level = risk; r.channel = channel
    r.payload_json = {"message_text": msg, "admin_alert_text": msg}
    r.approved_by = 999; r.approved_at = now
    r.rejected_by = None; r.rejected_at = None
    r.blocked_reason = None; r.created_at = now
    r.expires_at = now + timedelta(minutes=expires_minutes)
    r.executed_at = executed_at; r.failed_at = failed_at; r.last_error = None
    return r
...
class TestPresetFlags:
    ...
    def test_allowed_with_flag(self):
        r = AgentRolloutPresetService.preview_preset("approved_live_send", {}, allow_live_flags=True)
        assert r.allowed
...
class TestSendApproved:
    async def test_approved_sends(self):
        bot = AsyncMock(); bot.send_message = AsyncMock(return_value=MagicMock())
    ...
    async def test_executed_at_set(self):
        bot = AsyncMock(); bot.send_message = AsyncMock()
        r = await ApprovedExecutionSenderService.send_record(_rec(), bot)
...
class TestNoSendStatuses:
    async def test_proposed_no_send(self):
        ...
        assert r.blocked is True; bot.send_message.assert_not_called()
    ...
    async def test_failed_no_resend(self):
        bot = AsyncMock()
        r = await ApprovedExecutionSenderService.send_record(
            _rec(failed_at=datetime.now(UTC)), bot)
        assert r.blocked is True
...
class TestBotError:
    async def test_exception_marks_failed(self):
        bot = AsyncMock(); bot.send_message = AsyncMock(side_effect=Exception("network"))
        r = await ApprovedExecutionSenderService.send_record(_rec(), bot)
        assert r.status == "failed"

    async def test_error_sanitized(self):
        bot = AsyncMock(); bot.send_message = AsyncMock(side_effect=Exception("sk-secret err"))
        r = await ApprovedExecutionSenderService.send_record(_rec(), bot)
...
class TestRollback:
    def test_off_disables(self):
        ...
        s = AgentEffectiveSettingsService(off); s._runtime_enabled = True
