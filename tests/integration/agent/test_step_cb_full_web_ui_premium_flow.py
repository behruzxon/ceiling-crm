...
class TestActivePagesSet:
    ...
    def test_crm(self):
        assert "active_page" in Path("apps/web/templates/crm_contacts.html").read_text(encoding="utf-8")
    def test_security(self):
        assert "active_page" in Path("apps/web/templates/security.html").read_text(encoding="utf-8")
    def test_campaigns(self):
        assert "active_page" in Path("apps/web/templates/crm_campaigns.html").read_text(encoding="utf-8")
    def test_dashboard(self):
        assert "active_page" in Path("apps/web/templates/dashboard.html").read_text(encoding="utf-8")
