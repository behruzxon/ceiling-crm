...
class TestDocsContent:
    def test_preparation_has_passfail(self):
        text = Path("docs/AI_AGENT_SYSTEM/34_STAGE_2_DRY_RUN_PREPARATION.md").read_text(
            encoding="utf-8"
        )
    ...
    def test_test_script_has_scenarios(self):
        text = Path("docs/AI_AGENT_SYSTEM/35_STAGE_2_DRY_RUN_TEST_SCRIPT.md").read_text(
            encoding="utf-8"
        )
        assert "qancha" in text.lower() or "scenario" in text.lower()

    def test_template_has_date(self):
        text = Path("docs/AI_AGENT_SYSTEM/36_STAGE_2_DRY_RUN_OBSERVATION_TEMPLATE.md").read_text(
            encoding="utf-8"
        )
