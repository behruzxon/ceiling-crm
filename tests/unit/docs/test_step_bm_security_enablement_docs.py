...
class TestEnablementPlan:
    def test_contains_stages(self):
        content = Path("docs/AI_AGENT_SYSTEM/69_SECURITY_ENABLEMENT_PLAN.md").read_text(
            encoding="utf-8"
        )
...
class TestRollbackCard:
    def test_disable_session_auth(self):
        content = Path("docs/AI_AGENT_SYSTEM/71_SECURITY_ROLLBACK_CARD.md").read_text(
            encoding="utf-8"
        )
...
class TestRunbook:
    def test_how_to_run(self):
        content = Path("docs/AI_AGENT_SYSTEM/70_SECURITY_PREFLIGHT_RUNBOOK.md").read_text(
            encoding="utf-8"
        )
...
class TestStagingTestScript:
    def test_manual_scenarios(self):
        content = Path("docs/AI_AGENT_SYSTEM/72_SECURITY_STAGING_TEST_SCRIPT.md").read_text(
            encoding="utf-8"
        )
