...
class TestDocsExist:
    ...
    def test_template(self):
        assert Path(
            "docs/AI_AGENT_SYSTEM/51_STAGE_5_APPROVED_LIVE_SEND_OBSERVATION_TEMPLATE.md"
        ).exists()


class TestContent:
    def test_rollback(self):
        t = Path("docs/AI_AGENT_SYSTEM/49_STAGE_5_APPROVED_LIVE_SEND_PREPARATION.md").read_text(
            encoding="utf-8"
        )
