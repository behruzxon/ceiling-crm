...
class TestDocsExist:
    ...
    def test_template(self):
        assert Path(
            "docs/AI_AGENT_SYSTEM/46_STAGE_4_APPROVAL_REQUIRED_OBSERVATION_TEMPLATE.md"
        ).exists()


class TestContent:
    def test_rollback(self):
        t = Path("docs/AI_AGENT_SYSTEM/44_STAGE_4_APPROVAL_REQUIRED_PREPARATION.md").read_text(
            encoding="utf-8"
        )
