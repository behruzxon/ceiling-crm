...
class TestBlockers:
    ...
    def test_live_sender(self):
        assert not svc.evaluate_approval_to_live_send(
            _report(no_send=Stage4NoSendSafetyMetrics(live_sender_executed=1)), _SAFE
        ).allowed

    def test_auto_execute(self):
        assert not svc.evaluate_approval_to_live_send(
            _report(no_send=Stage4NoSendSafetyMetrics(auto_execute_count=1)), _SAFE
        ).allowed

    def test_user_dm(self):
        assert not svc.evaluate_approval_to_live_send(
            _report(no_send=Stage4NoSendSafetyMetrics(user_dm_sent_count=1)), _SAFE
        ).allowed

    def test_health_red(self):
        assert (
            "health_red"
            in svc.evaluate_approval_to_live_send(_report(health_status="red"), _SAFE).blockers
        )

    def test_no_proposals(self):
        assert (
            "no_proposals"
            in svc.evaluate_approval_to_live_send(_report(total_proposals=0), _SAFE).blockers
        )

    def test_no_approved(self):
        assert (
            "no_approved_samples"
            in svc.evaluate_approval_to_live_send(
    ...
    def test_stale_excessive(self):
        assert (
            "stale_pending_excessive"
            in svc.evaluate_approval_to_live_send(_report(stale_pending_count=25), _SAFE).blockers
        )

    def test_allow_live_false(self):
        assert (
            "allow_live_flags_false" in svc.evaluate_approval_to_live_send(_report(), {}).blockers
        )

    def test_mode_live(self):
        assert (
            "execution_mode_already_live"
            in svc.evaluate_approval_to_live_send(
                _report(), {**_SAFE, "agent_execution_mode": "live"}
            ).blockers
        )
...
class TestWarnings:
    def test_low_proposals(self):
        assert (
            "low_proposal_count"
            in svc.evaluate_approval_to_live_send(_report(total_proposals=10), _SAFE).warnings
        )

    def test_low_approved(self):
        assert (
            "low_approved_count"
            in svc.evaluate_approval_to_live_send(_report(approved_count=3), _SAFE).warnings
        )
    ...
    def test_high_rejection(self):
        assert (
            "high_rejection_rate"
            in svc.evaluate_approval_to_live_send(
                _report(total_proposals=20, rejected_count=18), _SAFE
            ).warnings
        )
    ...
    def test_high_expiration(self):
        assert (
            ...
            in svc.evaluate_approval_to_live_send(
                _report(total_proposals=20, expired_count=15), _SAFE
            ).warnings
        )
    ...
    def test_pending_high(self):
        assert (
            "pending_count_high"
            in svc.evaluate_approval_to_live_send(_report(pending_count=25), _SAFE).warnings
        )

    def test_yellow_health(self):
        assert (
            "health_yellow"
            in svc.evaluate_approval_to_live_send(_report(health_status="yellow"), _SAFE).warnings
        )

    def test_short_observation(self):
        assert (
            "short_observation"
            in svc.evaluate_approval_to_live_send(_report(duration_minutes=20), _SAFE).warnings
        )
...
class TestScore:
    ...
    def test_blocker_max_40(self):
        assert (
            svc.evaluate_approval_to_live_send(_report(health_status="red"), _SAFE).readiness_score
            <= 40
        )
    ...
    def test_clamped(self):
        r = svc.evaluate_approval_to_live_send(
            _report(
                no_send=Stage4NoSendSafetyMetrics(executed_records_count=1),
                pass_fail=Stage4PassFailResult(passed=False),
                health_status="red",
            ),
            _SAFE,
        )
...
class TestRecommendations:
    ...
    def test_allow_live_false(self):
        ...
        assert any(
            "allow_live" in rec.lower() or "live" in rec.lower() for rec in r.recommendations
        )

    def test_no_proposals(self):
        r = svc.evaluate_approval_to_live_send(_report(total_proposals=0), _SAFE)
        assert any(
            "proposal" in rec.lower() or "continue" in rec.lower() for rec in r.recommendations
        )

    def test_no_approved(self):
        r = svc.evaluate_approval_to_live_send(_report(total_proposals=10, approved_count=0), _SAFE)
