...
class TestSafetyCheckAPI:
    ...
    async def test_token_blocked(self):
        ...
        r = await safety_check(
            SafetyCheckBody(segment_key="hot_leads", message_text="sk-secret123")
        )
        assert not r["ok"]
...
class TestDraftsAPI:
    ...
    async def test_create_valid(self):
        ...
        r = await create_draft(
            DraftCreateBody(name="Test", segment_key="hot_leads", message_text="Salom!")
        )
        assert r["ok"]
    ...
    async def test_create_invalid_segment(self):
        ...
        r = await create_draft(
            DraftCreateBody(name="Test", segment_key="bad", message_text="Salom")
        )
        assert not r["ok"]
    ...
    async def test_create_token_blocked(self):
        ...
        r = await create_draft(
            DraftCreateBody(name="Test", segment_key="hot_leads", message_text="sk-secret123")
        )
        assert not r["ok"]
