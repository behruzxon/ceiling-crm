"""
core.services.ai_engine
~~~~~~~~~~~~~~~~~~~~~~~
Channel-agnostic OpenAI integration layer.

Contains the OpenAI client, context helpers, DB persistence helpers,
and the main ``_call_ai`` function.  No aiogram or Telegram imports.

Both the Telegram bot (``apps/bot``) and the web API (``apps/api``) import
from this module.  ``apps/bot/handlers/private/ai_openai.py`` re-exports
...
# Minimal fallback system prompt used only when the caller passes no
# system_prompt.  In practice every caller (bot + API) provides a
# tenant-configured prompt, so this is a last-resort safety net.
_SYSTEM_PROMPT_FALLBACK = (
    "Sen yordamchi AI assistent. Foydalanuvchining savollariga o'zbek tilida "
    "qisqa va aniq javob ber. Javobni JSON formatida qaytar: "
    '{\"reply\": \"...\", \"intent\": \"other\"}'
)
    ...
    *,
    ...
    try:
        ...
        if tenant_id is not None and rl.tenant_daily_limit > 0:
            ...
            count = await cache.incr_with_ttl(quota_key, CacheTTL.AI_DAILY_QUOTA)
...
async def _load_context(
    ...
    try:
        ...
        async with factory() as session:
            mem = (
                await session.get(AiMemoryModel, (tenant_id, user_id))
            ...
            conv = (
                await session.get(AiConversationModel, (tenant_id, user_id))
                if tenant_id is not None
                else None
            )
        ...
        log.warning("ai_context_load_failed", user_id=user_id, tenant_id=tenant_id)
        return {}, [], None
...
async def _persist_exchange(
    *,
    ...
    if tenant_id is None:
        log.warning("ai_persist_exchange_no_tenant", user_id=user_id)
        return
    try:
        new_messages = (
            current_messages
        ...
        async with factory() as session:
            ...
            await session.execute(
                ...
                .values(tenant_id=tenant_id, user_id=user_id, profile=new_profile)
                .on_conflict_do_update(
                    index_elements=["tenant_id", "user_id"],
                    set_={
                        "profile": new_profile,
...
async def clear_ai_conversation(user_id: int, tenant_id: int | None = None) -> None:
    ...
    """
    if tenant_id is None:
        log.warning("clear_ai_conversation_no_tenant", user_id=user_id)
        return
    try:
        factory = get_session_factory()
        async with factory() as session:
            await session.execute(
                pg_insert(AiConversationModel)
                .values(tenant_id=tenant_id, user_id=user_id, last_messages=[], summary=None)
                .on_conflict_do_update(
                    index_elements=["tenant_id", "user_id"],
                    set_={
                        "last_messages": [],
            ...
            await session.commit()
        log.info("ai_conversation_cleared", user_id=user_id, tenant_id=tenant_id)
    except Exception:
...
async def _store_user_message_only(
    *,
    ...
    if tenant_id is None:
        log.warning("ai_store_user_msg_no_tenant", user_id=user_id)
        return
    try:
        new_messages = (
        ...
        async with factory() as session:
            await session.execute(
                pg_insert(AiConversationModel)
                .values(tenant_id=tenant_id, user_id=user_id, last_messages=new_messages)
                .on_conflict_do_update(
                    index_elements=["tenant_id", "user_id"],
                    set_={
                        "last_messages": new_messages,
...
async def _call_ai(
    ...
    Args:
        system_prompt: Tenant-specific prompt.  Falls back to the hardcoded
                       ``_SYSTEM_PROMPT_FALLBACK`` when *None*.
    ...
    messages: list[dict[str, str]] = [
        {"role": "system", "content": (system_prompt or _SYSTEM_PROMPT_FALLBACK) + _SAFETY_SUFFIX},
    ]
    if context_block:
...
def _detect_intent(text: str) -> str:
    """Heuristic intent detection from plain-text AI reply.

    Used by the streaming endpoint where OpenAI JSON mode is unavailable.
    Matches common Uzbek, Russian, and English keywords.
    """
    t = text.lower()
    if any(k in t for k in ("o'lchov", "olchov", "measurement", "замер", "o`lchov")):
        return "measurement"
    if any(k in t for k in ("operator", "mutaxassis", "мутахассис", "оператор", "specialist")):
        return "operator"
    if any(k in t for k in ("narx", "цена", "price", "cost", "narxi")):
        return "price"
    if any(k in t for k in ("katalog", "catalog", "каталог", "dizayn")):
        return "catalog"
    if any(k in t for k in ("kafolat", "гарантия", "warranty", "sifat")):
        return "faq"
    return "other"
...
async def generate_stream(
    ...
    """Yield AI reply tokens as plain strings (streaming mode).

    Unlike ``_call_ai``, this function does NOT use
    ``response_format={"type":"json_object"}`` — that option is incompatible
    with ``stream=True``.  The system prompt instructs the AI to respond in
    plain Uzbek text; intent is classified post-hoc by ``_detect_intent``.

    Raises on connection/API errors so the caller can handle gracefully.
    """
    from shared.utils.prompt_safety import sanitize_history, sanitize_user_message
    ...
    # Plain-text streaming prompt suffix (replaces JSON format instruction)
    _STREAM_SUFFIX = (
        "\n\nFaqat oddiy matn bilan javob ber. JSON formatida yozma. "
    ...
    messages: list[dict[str, str]] = [
        {
            "role": "system",
            "content": (system_prompt or _SYSTEM_PROMPT_FALLBACK) + _SAFETY_SUFFIX + _STREAM_SUFFIX,
        },
    ]
    if context_block:
        messages.append({"role": "system", "content": context_block})
