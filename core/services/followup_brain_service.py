...
def decide_follow_up(
    *,
    ...
    followup_weights: dict[str, float] | None = None,
) -> FollowUpDecision:
    ...
    fu_type, reason = _select_type(
        ...
        followup_weights=followup_weights,
    )
...
def _avoid_repeat(
    ...
    When *followup_weights* are provided (from outcome-based learning),
    pick the highest-weighted fallback instead of the hardcoded chain.
    """
    if previous and preferred == previous:
        ...
        if followup_weights:
            # Pick the best-weighted alternative (exclude previous)
            candidates = [t for t in _fallback if t != previous]
            if candidates:
                rotated = max(candidates, key=lambda t: followup_weights.get(t, 1.0))
