...
async def _check_and_set_cooldown(user_id: int, *, bot_id: int | None = None) -> bool:
    ...
    try:
        ...
        ok = await get_redis().set(
            CacheKeys.sales_closer_last(user_id, bot_id=bot_id),
            "1",
...
async def attempt_close(
    ...
    *,
    ...
    try:
        ...
        memory = await _load_ai_memory(user_id, bot_id=_bot_id)
        ...
        if not await _check_and_set_cooldown(user_id, bot_id=_bot_id):
        ...
        await _save_ai_memory(user_id, memory, bot_id=_bot_id)
